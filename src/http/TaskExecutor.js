...
function resolveSyncTarget(form, source, instance) {
  ...
  if (target && instance) {
    target = target.replace(/\{id\}/g, () => {
      ...
      return id !== null && id !== undefined && id !== '' ? encodeURIComponent(String(id)) : '';
    });
...
async function asyncFetch(form, url, verb, descriptor, instance) {
  const capturedState = instance && typeof instance.state === 'function' ? instance.state() : null;
...
function execute(form, descriptor, options = {}) {
  ...
  if (!instance) {
    throw new Error('[SmartForms] form not registered; call registerForm(form, instance) first');
  }
